...
- STATEMENT：记录sql语句，但会有动态函数问题
- ROW：可能文件很大
- MIXED：自动使用以上两种
而redo log是物理日志，记录某个数据页做了什么修改


写入方式：
binlog：追加写,写满一个接着新建写；
redo log：循环写，写满就从头开始，保存未被刷入磁盘的脏页日志。


**q**:如果不小心整个数据库的数据被删除了，能使用 redo log 文件恢复数据吗？
只能用binlog。
redo log只会记录未被刷入磁盘的数据的物理日志。
...
应用：全库逻辑备份；（例子：一个业务操作涉及多表修改，但是备份的时候假如备份的分别是a表修改前和b表修改后被备份，就会出现问题）
缺点：全局锁导致整个数据库为**只读**,影响业务；
改进：**可重复读的隔离级别**：创建Read View，期间的备份基于这个快照，而该期间业务可以继续更新。

mysqldump ：-single-transaction

### 表级锁
尽量避免使用->行级锁 is better；
#### 元数据锁MDL
...
对某些**记录**:
先加意向共享锁(表级别)，再加共享锁
先加意向独占锁（表级别），然后对读取记录加独占锁
意向锁有什么作用：
首先，这个锁不会和行级的锁发生冲突，但他会和表锁产生冲突，所以在想加行级锁时，先加上意向锁，这时候如果再想加表级锁，就可以快速遍历是否有行级锁。**即为了快速判断表里是否有记录被加锁**。
...
select ... for update;
```
Record Lock:记录锁，仅锁一条记录
Gap Lock:间隙锁，锁定一个范围但不包括记录本身
Next-Key Lock:以上两者组合。锁范围且锁记录本身

Gap Lock:解决幻读

一个问题：
update没加索引，就会进行全表扫描，就会给每条记录加上锁（no 表锁）。
